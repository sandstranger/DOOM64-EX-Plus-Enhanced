...
// 
// A_LookCheckreworkedvanillamonstersprites
//

void A_LookCheckreworkedvanillamonstersprites(mobj_t* actor) {

	if (m_reworkedmonsters.value == 1)
	{
		// reworked vanilla monsters sprites
		if (actor->type == MT_POSSESSED2) {
			actor->info->palette = 0;
			
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_POSS2_STND0REWORK);
			}
			...
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_TROO_STND0REWORK);
			}
			...
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_BOSS1_STND0REWORK);
			}
			...
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_BOSS2_STND0REWORK);
			}
			...
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_SARG_STND0REWORK);
			}
...
// 
// A_ChaseCheckreworkedvanillamonstersprites
//

void A_ChaseCheckreworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_POSS2_RUN0REWORK);
			}
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_TROO_RUN0REWORK);
			}
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_BOSS1_RUN0REWORK);
			}
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_BOSS2_RUN0REWORK);
			}
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_SARG_RUNAREWORK);
			}
...
// 
// A_PainCheckreworkedvanillamonstersprites
//

void A_PainCheckreworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_POSS2_PAIN0REWORK);
			}
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_TROO_PAIN0REWORK);
			}
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_BOSS1_PAIN0REWORK);
			}
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_BOSS2_PAIN0REWORK);
			}
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_SARG_PAINAREWORK);
			}
...
// 
// A_MeleeCheckreworkedvanillamonstersprites
//

void A_MeleeCheckreworkedvanillamonstersprites(mobj_t* actor) {
	...
	{
		// reworked vanilla monsters sprites
		if (actor->type == MT_IMP2) {
			actor->info->palette = 0;


			if (actor->info->meleestate) {
				P_SetMobjState(actor, S_TROO_MELEE0REWORK);
			}
			...
			if (actor->info->meleestate) {
				P_SetMobjState(actor, S_SARG_ATKAREWORK);
			}
...
// 
// A_AttackCheckreworkedvanillamonstersprites
//

void A_AttackCheckreworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->missilestate) {
				P_SetMobjState(actor, S_POSS2_ATK0REWORK);
			}
			...
			if (actor->info->missilestate) {
				P_SetMobjState(actor, S_TROO_ATK0REWORK);
			}
			...
			if (actor->info->missilestate) {
				P_SetMobjState(actor, S_BOSS1_ATK0REWORK);
			}
			...
			if (actor->info->missilestate) {
				P_SetMobjState(actor, S_BOSS2_ATK0REWORK);
			}
...
// 
// A_DieCheckreworkedvanillamonstersprites
//

void A_DieCheckreworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_POSS2_DIE0REWORK);
			}
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_TROO_DIE0REWORK);
			}
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_BOSS1_DIE0REWORK);
			}
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_BOSS2_DIE0REWORK);
			}
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_SARG_DIEAREWORK);
			}
...
// 
// A_XdieCheckreworkedvanillamonstersprites
//

void A_XdieCheckreworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->xdeathstate) {
				P_SetMobjState(actor, S_POSS2_XDIE0REWORK);
			}
			...
			if (actor->info->xdeathstate) {
				P_SetMobjState(actor, S_TROO_XDIE0REWORK);
			}
...
// 
// A_RaiseCheckreworkedvanillamonstersprites
//

void A_RaiseCheckreworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_POSS2_RAISE0REWORK);
			}
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_TROO_RAISE0REWORK);
			}
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_BOSS1_RAISE0REWORK);
			}
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_BOSS2_RAISE0REWORK);
			}
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_SARG_RAISEAREWORK);
			}
...
// 
// A_LookCheckvanillamonstersprites
//

void A_LookCheckvanillamonstersprites(mobj_t* actor) {

	if (m_reworkedmonsters.value == 0)
	{
		// vanilla monsters sprites
		if (actor->type == MT_POSSESSED2) {
			actor->info->palette = 1;

			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_POSS2_STND0);
			}
			...
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_TROO_STND0);
			}
			...
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_BOSS1_STND0);
			}
			...
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_BOSS2_STND0);
			}
			...
			if (actor->info->spawnstate) {
				P_SetMobjState(actor, S_SARG_STND0);
			}
...
// 
// A_ChaseCheckvanillamonstersprites
//

void A_ChaseCheckvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_POSS2_RUN0);
			}
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_TROO_RUN0);
			}
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_BOSS1_RUN0);
			}
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_BOSS2_RUN0);
			}
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_SARG_RUNA);
			}
...
// 
// A_PainCheckvanillamonstersprites
//

void A_PainCheckvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_POSS2_PAIN0);
			}
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_TROO_PAIN0);
			}
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_BOSS1_PAIN0);
			}
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_BOSS2_PAIN0);
			}
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_SARG_PAINA);
			}
...
// 
// A_MeleeCheckvanillamonstersprites
//

void A_MeleeCheckvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_IMP2) {
			...
			if (actor->info->meleestate) {
				P_SetMobjState(actor, S_TROO_MELEE0);
			}
			...
			if (actor->info->meleestate) {
				P_SetMobjState(actor, S_SARG_ATKA);
			}
...
// 
// A_AttackCheckvanillamonstersprites
//

void A_AttackCheckvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->missilestate) {
				P_SetMobjState(actor, S_POSS2_ATK0);
			}
			...
			if (actor->info->missilestate) {
				P_SetMobjState(actor, S_TROO_ATK0);
			}
			...
			if (actor->info->missilestate) {
				P_SetMobjState(actor, S_BOSS1_ATK0);
			}
			...
			if (actor->info->missilestate) {
				P_SetMobjState(actor, S_BOSS2_ATK0);
			}
...
// 
// A_DieCheckvanillamonstersprites
//

void A_DieCheckvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_POSS2_DIE0);
			}
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_TROO_DIE0);
			}
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_BOSS1_DIE0);
			}
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_BOSS2_DIE0);
			}
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_SARG_DIEA);
			}
...
// 
// A_XdieCheckvanillamonstersprites
//

void A_XdieCheckvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->xdeathstate) {
				P_SetMobjState(actor, S_POSS2_XDIE0);
			}
			...
			if (actor->info->xdeathstate) {
				P_SetMobjState(actor, S_TROO_XDIE0);
			}
...
// 
// A_RaiseCheckvanillamonstersprites
//

void A_RaiseCheckvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_POSSESSED2) {
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_POSS2_RAISE0);
			}
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_TROO_RAISE0);
			}
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_BOSS1_RAISE0);
			}
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_BOSS2_RAISE0);
			}
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_SARG_RAISEA);
			}
...
// 
// A_ChaseCheckspectrereworkedvanillamonstersprites
//

void A_ChaseCheckspectrereworkedvanillamonstersprites(mobj_t* actor) {

	if (m_reworkedmonsters.value == 1)
	{
		// reworked vanilla monsters sprites
		if (actor->type == MT_DEMON2) {
			actor->info->palette = 0;

			if (actor->info->seestate) {
				P_SetMobjState(actor, S_SARG_RUNBREWORK);
			}
...
// 
// A_PainCheckspectrereworkedvanillamonstersprites
//

void A_PainCheckspectrereworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_SARG_PAINBREWORK);
			}
...
// 
// A_MeleeCheckspectrereworkedvanillamonstersprites
//

void A_MeleeCheckspectrereworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->meleestate) {
				P_SetMobjState(actor, S_SARG_ATKBREWORK);
			}
...
// 
// A_DieCheckspectrereworkedvanillamonstersprites
//

void A_DieCheckspectrereworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_SARG_DIEBREWORK);
			}
...
// 
// A_RaiseCheckspectrereworkedvanillamonstersprites
//

void A_RaiseCheckspectrereworkedvanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 1)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_SARG_RAISEBREWORK);
			}
...
// 
// A_ChaseCheckspectrevanillamonstersprites
//

void A_ChaseCheckspectrevanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->seestate) {
				P_SetMobjState(actor, S_SARG_RUNB);
			}
...
// 
// A_PainCheckspectrevanillamonstersprites
//

void A_PainCheckspectrevanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->painstate) {
				P_SetMobjState(actor, S_SARG_PAINB);
			}
...
// 
// A_MeleeCheckspectrevanillamonstersprites
//

void A_MeleeCheckspectrevanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->meleestate) {
				P_SetMobjState(actor, S_SARG_ATKB);
			}
...
// 
// A_DieCheckspectrevanillamonstersprites
//

void A_DieCheckspectrevanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->deathstate) {
				P_SetMobjState(actor, S_SARG_DIEB);
			}
...
// 
// A_RaiseCheckspectrevanillamonstersprites
//

void A_RaiseCheckspectrevanillamonstersprites(mobj_t* actor) {
	...
	if (m_reworkedmonsters.value == 0)
	{
		...
		if (actor->type == MT_DEMON2) {
			...
			if (actor->info->raisestate) {
				P_SetMobjState(actor, S_SARG_RAISEB);
			}
